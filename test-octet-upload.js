...
async function testOctetStream() {
    console.log("--- TESTING OCTET-STREAM UPLOAD ---");

    // 1. Create a dummy tiny mp4 file
    const tempVideoPath = path.join(__dirname, 'test_octet.mp4');
    ...
    // 2. Storage Upload with application/octet-stream
    const fileName = `${campaignId}/octet-${Date.now()}.mp4`;
    const filePath = `assets/${fileName}`;

    console.log(`Uploading as application/octet-stream: ${filePath}...`);
    ...
    console.log('Storage upload successful with octet-stream.');
    ...
    const { data: dbData, error: dbError } = await supabase
        ...
        .insert({
            ...
            asset_url: publicUrl,
            metadata: {
                original_name: 'test_octet.mp4',
                size_bytes: fileStats.size,
                mime_type: 'video/mp4', // We still label it as video in DB
                manual_upload: true
            }
        })
        .select();
